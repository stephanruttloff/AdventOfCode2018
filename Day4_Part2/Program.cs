...
namespace Day4_Part2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var sleepyGuards = sleep.Select(x => new { Guard = x.Key, SleepCycle = x.Value, SleepMaxMin = x.Value.Max() })
                .OrderByDescending(x => x.SleepMaxMin);
            var sleepyGuard = sleepyGuards.First();
            var sleepyGuardMinute = sleepyGuard.SleepCycle.ToList().IndexOf(sleepyGuard.SleepCycle.Max());

            Console.WriteLine($"Guard #{sleepyGuard.Guard} was asleep the most at minute {sleepyGuardMinute}.");
